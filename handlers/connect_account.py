...
async def handle_connect_account(message: Message, state: FSMContext):
    ...
    Args:
        ...
        state (FSMContext): Контекст машины состояний, используется для сброса текущего состояния.
...
async def handle_account_file(message: Message, state: FSMContext):
    ...
    Args:
        message (Message): Входящее сообщение с документом от пользователя.
        state (FSMContext): Контекст машины состояний, используется для сброса состояния.
