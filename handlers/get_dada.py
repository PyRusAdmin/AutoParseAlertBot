...
def create_excel_file(data: list, headers: list, filename: str, sheet_name: str) -> str:
    ...
    Args:
        data (list): Список кортежей, представляющих строки таблицы (например, [(1, 'слово'), ...]).
        headers (list): Список строк, представляющих заголовки столбцов (например, ['№', 'Ключевое слово']).
        filename (str): Имя создаваемого файла (например, "keywords_12345_20250420.xlsx").
        sheet_name (str): Название листа в Excel-файле.

    Returns:
        str: Абсолютный путь к созданному Excel-файлу.
...
async def get_keywords_list(message: Message, state: FSMContext):
    ...
    Args:
        message (Message): Входящее сообщение от пользователя, инициировавшего экспорт.
        state (FSMContext): Контекст машины состояний, сбрасывается в начале обработки.

    Returns:
        None

    Raises:
        Exception: Перехватывается локально при ошибках создания или отправки файла.
