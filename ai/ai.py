    ...
    try:
        chat_completion = await client_groq.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"Придумай 35 вариаций названий для групп {user_input}. Ответ дать строго наименования в столбик, без перечисления и без пояснения. 1. 2. 3. не применяй"
                }
            ],
        )
